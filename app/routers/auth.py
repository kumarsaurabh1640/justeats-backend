...
async def register(payload: UserCreate, db: AsyncSession = Depends(get_db)):
    ...
    # Pre-populate the profile with the name and phone provided at sign-up
...
async def token(form: OAuth2PasswordRequestForm = Depends(), db: AsyncSession = Depends(get_db)):
    """OAuth2-compatible endpoint used by Swagger UI Authorize. `username` is the email address."""
...
async def forgot_password(payload: ForgotPasswordRequest, db: AsyncSession = Depends(get_db)):
    """
    Send password reset token. In production, email the token to the user.
    ...
    if user and user.is_active:
        ...
        # structlog would log for production debugging
        if settings.DEBUG:
            return {
