...
- Restaurant Owners: View and manage order status
"""
...
async def _load_order_with_items(order_id: uuid.UUID, db: AsyncSession) -> Order:
    """Load an order with its items eagerly loaded.
    ...
    Returns:
        The order with order_items populated.
...
async def place_order(
    ...
    Validates all menu items are available and belong to the restaurant,
    then creates the order with calculated totals.

    Args:
        payload: Order details including restaurant, items, and instructions.

    Returns:
        The newly created order with items.

    Raises:
        HTTPException: 404 if restaurant not found, 400 if item invalid.
    """
...
async def list_my_orders(
    ...
    """Get the current customer's order history (Customer only).

    Returns:
        List of orders sorted by newest first.
    """
    profile = await get_customer_profile(current_user, db)
...
async def list_restaurant_orders(
    ...
    Only the restaurant owner can view orders.
    ...
    Raises:
        HTTPException: 403 if not the restaurant owner.
    """
...
async def get_order(
    ...
    Owners can only view orders from their restaurants.

    Args:
        order_id: The order's unique identifier.

    Returns:
        The order details with items.
...
async def update_order_status(
    ...
    When marking as COMPLETED, increments order_count on menu items.
    ...
    Raises:
        HTTPException: 403 if not owner, 422 if invalid transition.
    """
    order = await _load_order_with_items(order_id, db)
